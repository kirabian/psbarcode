<div class="p-4 md:p-6 bg-zinc-50 min-h-screen font-sans">
                                ...
                                <div class="bg-white p-4 rounded-2xl shadow-sm border border-zinc-200">
                                    ...
                                    <div class="flex gap-2">
                                        <button wire:click="checkIcloud('{{ $pair[0] }}')"
                                            class="flex-1 py-2.5 rounded-xl text-[10px] font-bold transition-all {{ isset($icloudStatus[$pair[0]]) && $icloudStatus[$pair[0]]['status'] == 'ON' ? 'bg-emerald-500 text-white' : (isset($icloudStatus[$pair[0]]) && $icloudStatus[$pair[0]]['status'] == 'OFF' ? 'bg-red-500 text-white' : 'bg-zinc-100 text-zinc-700') }}">{{ $icloudStatus[$pair[0]]['status'] ?? 'CHECK A' }}</button>
                                        <button wire:click="checkIcloud('{{ $pair[1] }}')"
                                            class="flex-1 py-2.5 rounded-xl text-[10px] font-bold transition-all {{ isset($icloudStatus[$pair[1]]) && $icloudStatus[$pair[1]]['status'] == 'ON' ? 'bg-emerald-500 text-white' : (isset($icloudStatus[$pair[1]]) && $icloudStatus[$pair[1]]['status'] == 'OFF' ? 'bg-red-500 text-white' : 'bg-zinc-100 text-zinc-700') }}">{{ $icloudStatus[$pair[1]]['status'] ?? 'CHECK B' }}</button>
                                        @if ($viewMode == 'card')
                                            <button
    ...
    @if ($showModal && $selectedItem)
        <div
            class="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md p-2 md:p-4 overflow-y-auto">
            <div class="relative w-full max-w-sm my-auto">
                <div class="flex justify-between mb-4 px-2">
                    <button onclick="downloadCardPng()"
                        class="px-5 py-3 bg-emerald-500 text-white rounded-2xl font-bold text-xs shadow-lg hover:bg-emerald-600 transition-all active:scale-95">Download
                        PNG</button>
                    <button wire:click="closeModal"
                        class="px-5 py-3 bg-white/10 text-white rounded-2xl font-bold text-xs hover:bg-white/20 transition-all">Close</button>
                </div>
                <div id="capture-area"
                    class="rounded-[50px] overflow-hidden shadow-2xl flex justify-center bg-transparent scale-90 sm:scale-100">
    ...
    <script>
        ...
        async function downloadZip(type) {
            ...
            try {
                ...
                alert('Error: Memory Full atau Connection Lost.');
            } finally {
                restoreButtons();
            }
