@php
    ...
    if ($isLow) { $battFillColor = '#FF3B30'; } // Merah
    elseif ($isSaving) { $battFillColor = '#FFD60A'; } // Kuning (Low Power Mode)
    else { $battFillColor = '#34C759'; } // Hijau
    ...
    } else { $finalEid = substr($rawEid, 0, 33); }
@endphp
...
<div id="{{ $id }}" class="iphone-screen" style="width: 375px; height: 812px; background-color: {{ $bgMain }}; color: {{ $textColor }}; font-family: -apple-system, BlinkMacSystemFont, sans-serif; position: relative; overflow: hidden; flex-shrink: 0; box-sizing: border-box; -webkit-font-smoothing: antialiased;">
    ...
    <div style="display: flex; justify-content: space-between; padding: 10px 26px 0 26px; align-items: center; height: 44px; position: absolute; top: 0; left: 0; width: 100%; z-index: 50; box-sizing: border-box;">
        <div style="font-weight: 600; font-size: 15px; width: 54px; text-align: left; color: {{ $headerColor }}; transform: translateX(8px); margin-top: -4px;">
        ...
        <div style="display: flex; gap: 7px; align-items: center; margin-top: -12px;">
            ...
            <div style="position: relative; width: 25px; height: 12px;">
                <svg width="25" height="12" viewBox="0 0 25 12">
                    ...
                    <text x="11" y="6.5" font-family="sans-serif" font-size="6.5" font-weight="700" fill="{{ $battTextColor }}" text-anchor="middle" dominant-baseline="middle">{{ $battLevel }}</text>
                </svg>
            </div>
        </div>
    </div>

    <div style="position: absolute; bottom: 0; left: 50%; transform: translateX(-50%); width: 94%; height: 92%; background-color: {{ $bgBack }}; border-top-left-radius: 0px; border-top-right-radius: 0px; z-index: 8;"></div>

    <div style="position: absolute; bottom: 0; left: 0; width: 100%; height: 90%; background-color: {{ $bgCard }}; border-top-left-radius: 0px; border-top-right-radius: 0px; z-index: 10; display: flex; flex-direction: column; overflow: hidden;">
